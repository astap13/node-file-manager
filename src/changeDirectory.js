...
export function changeDirectory(newDir) {
    if (!newDir || typeof newDir !== 'string') {
        console.log('Operation failed: new directory path is required and should be a string.');
        return;
    }
    ...
    if (!statSync(newPath).isDirectory()) {
        console.log(`Operation failed: path "${newPath}" exists but is not a directory.`);
        return;
    }
